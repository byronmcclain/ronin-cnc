...
//! It uses a 64-bit block size and variable key length (1-56 bytes).
...
impl BlowfishEngine {
    ...
    /// Initialize with a key (1-56 bytes)
    ///
    /// This is a time-consuming operation as it involves encrypting
    /// all zeros to generate the subkey schedule.
    pub fn set_key(&mut self, key: &[u8]) {
        ...
        assert!(key.len() <= Self::MAX_KEY_LENGTH);

        // Reset to initial values
        self.p_encrypt = P_INIT;
        self.s_boxes = S_INIT;

        // XOR P-box with key bytes cyclically
        let mut key_idx = 0;
        for i in 0..18 {
            let mut data = 0u32;
            for _ in 0..4 {
        ...
        // Generate P-box values by encrypting zeros
        let mut left = 0u32;
        let mut right = 0u32;
    ...
    /// Sub-key encryption for key schedule generation
    ///
    /// This is the same as process_block but without byte order conversion
    /// since we're working with native integers.
    fn sub_key_encrypt(&self, left: &mut u32, right: &mut u32) {
        ...
        *left = r ^ self.p_encrypt[Self::ROUNDS + 1];
        *right = l ^ self.p_encrypt[Self::ROUNDS];
    }
    ...
    fn process_block(&self, input: &[u8; 8], output: &mut [u8; 8], p_table: &[u32; 18]) {
        ...
        // Final XOR (note: left and right are swapped in output)
    ...
    ///
    /// Data must be a multiple of 8 bytes. Returns number of bytes processed.
    pub fn encrypt(&self, data: &mut [u8]) -> usize {
        ...
        for i in 0..block_count {
            ...
            input.copy_from_slice(&data[offset..offset + 8]);
            self.process_block(&input, &mut output, &self.p_encrypt);
            data[offset..offset + 8].copy_from_slice(&output);
        }
    ...
    pub fn decrypt(&self, data: &mut [u8]) -> usize {
        ...
        for i in 0..block_count {
            ...
            input.copy_from_slice(&data[offset..offset + 8]);
            self.process_block(&input, &mut output, &self.p_decrypt);
            data[offset..offset + 8].copy_from_slice(&output);
        }
    ...
    /// Check if the engine has been keyed
...
// =============================================================================
// Initial P-box and S-box values (from digits of Pi)
// Copied from CODE/BLOWFISH.CPP
// =============================================================================
...
mod tests {
    ...
    fn test_roundtrip() {
        ...
        let original = *b"Hello!!X"; // 8 bytes
    ...
    fn test_roundtrip_multiple_blocks() {
        let mut bf = BlowfishEngine::new();
        bf.set_key(b"TESTKEY123");

        let original = *b"Hello World! This is a test!!!!!"; // 32 bytes
        let mut data = original;

        let enc_len = bf.encrypt(&mut data);
        assert_eq!(enc_len, 32);
        assert_ne!(data, original);

        let dec_len = bf.decrypt(&mut data);
    ...
    fn test_different_keys_different_output() {
        let mut bf1 = BlowfishEngine::new();
        let mut bf2 = BlowfishEngine::new();
        ...
        bf1.encrypt(&mut data1);
        bf2.encrypt(&mut data2);
