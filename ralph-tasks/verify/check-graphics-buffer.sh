...
echo "[2/4] Building TestGraphicsBuffer..."
if ! cmake --build build --target TestGraphicsBuffer 2>&1 | tail -20; then
...
# Step 4: Run tests (quick mode to skip visual tests)
echo "[4/4] Running unit tests..."
if ./build/TestGraphicsBuffer --quick 2>&1 | tee /tmp/gb_test_output.txt | grep -q "All tests PASSED"; then
    echo "  OK: Tests passed"
else
    echo "Test output:"
    cat /tmp/gb_test_output.txt
