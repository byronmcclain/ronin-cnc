# Task 08c: ADPCM Decoder

## Dependencies
- Task 08b must be complete (PCM playback)
- Understanding of IMA ADPCM format

## Context
Red Alert uses IMA ADPCM compressed audio in its AUD files for memory efficiency. The decoder is in `CODE/ADPCM.CPP`. This task implements the ADPCM decoder in Rust and provides FFI functions to create sounds from ADPCM data.

## Objective
Implement a complete IMA ADPCM decoder that converts compressed audio to PCM samples, then provide an FFI function to create sounds directly from ADPCM data.

## Deliverables
- [ ] `platform/src/audio/adpcm.rs` - Complete ADPCM decoder implementation
- [ ] FFI export for creating sounds from ADPCM data
- [ ] Unit tests for ADPCM decoder
- [ ] Verification passes

## Files to Modify

### platform/src/audio/adpcm.rs (REPLACE)
```rust
//! IMA ADPCM decoder
//!
...
Add ADPCM FFI export after sound functions:
```rust
...
cargo test --manifest-path platform/Cargo.toml -- adpcm 2>&1 && \
cargo build --manifest-path platform/Cargo.toml --features cbindgen-run 2>&1 && \
grep -q "Platform_Sound_CreateFromADPCM" include/platform.h && \
echo "VERIFY_SUCCESS"
```

## Implementation Steps
1. Replace `platform/src/audio/adpcm.rs` with full implementation
2. Add unit tests for ADPCM decoder
3. Add ADPCM FFI exports to `platform/src/ffi/mod.rs`
4. Run ADPCM unit tests
5. Build with cbindgen
6. Verify header contains ADPCM functions
7. Run verification command

## Completion Promise
When verification passes, output:
```
<promise>TASK_08C_COMPLETE</promise>
```

## Escape Hatch
If stuck after 12 iterations:
- Document what's blocking in `ralph-tasks/blocked/08c.md`
- List attempted approaches
- Output: `<promise>TASK_08C_BLOCKED</promise>`
