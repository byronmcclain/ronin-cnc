# Task 10d: Packet Types and Serialization

## Dependencies
- Tasks 10a-10c must be complete (network module, host, client)

## Context
The original Red Alert game used a structured packet format for multiplayer communication. This task defines the packet types and provides serialization/deserialization functions to convert between game structures and raw bytes.

**Original Protocol Analysis** (from SESSION.H and CONNECT.H):
- `CommHeaderType`: 7-byte header with magic number, code, packet ID
- `GlobalPacketType`: Main network packet with command and player info
- `SerialPacketType`: Serial/modem packet format
- Various command enums for game state

## Objective
Create packet types compatible with the original game protocol:
1. `GamePacketHeader` - Packet header structure
2. `GamePacketKind` - Packet type enumeration
3. `PacketType` - Delivery mode (reliable/unreliable)
4. Serialization/deserialization functions
5. FFI exports for packet construction

## Deliverables
- [ ] `platform/src/network/packet.rs` - Packet definitions
- [ ] `serialize_packet()` and `deserialize_packet()` functions
- [ ] FFI exports for packet operations
- [ ] Unit tests for serialization round-trips
- [ ] Verification passes

## Technical Background

### Original Packet Structure (CONNECT.H)
```cpp
typedef struct {
    unsigned short MagicNumber;    // 2 bytes - validation
    unsigned char Code;            // 1 byte - packet type
    unsigned long PacketID;        // 4 bytes - sequence number
} CommHeaderType;                  // Total: 7 bytes
```

### Packet Codes (from ConnectionClass)
- `PACKET_DATA_ACK` (0): Data requiring acknowledgment (reliable)
- `PACKET_DATA_NOACK` (1): Data not requiring ack (unreliable)
- `PACKET_ACK` (2): Acknowledgment packet

### Network Commands (SESSION.H NetCommandType)
Various game-specific commands for joining, chatting, syncing game state.

### Channel Mapping
- Channel 0: Reliable delivery (game state, chat)
- Channel 1: Unreliable delivery (position updates, time-sensitive data)

## Files to Create/Modify

### platform/src/network/packet.rs (NEW)
```rust
...
#[no_mangle]
pub unsafe extern "C" fn Platform_Packet_ValidateHeader(
    data: *const u8,
    size: i32,
) -> i32 {
...
pub use packet::{
    DeliveryMode, GamePacketHeader, GamePacketCode, GamePacket,
    PACKET_MAGIC, serialize_packet, deserialize_packet,
};
```

## Verification Command
```bash
ralph-tasks/verify/check-packet-types.sh
```

## Verification Script
Create `ralph-tasks/verify/check-packet-types.sh`:
```bash
#!/bin/bash
...
1. Create `platform/src/network/packet.rs`
2. Update `platform/src/network/mod.rs` to export packet module
3. Build and run tests
4. Verify FFI exports
5. Run verification script

## Success Criteria
- Packet serialization/deserialization round-trips correctly
- Header validation works
- FFI functions are null-safe
- All unit tests pass

## Common Issues

### "packed struct" alignment issues
The `#[repr(C, packed)]` attribute may cause unaligned access warnings. We handle this by using explicit byte operations rather than direct struct field access.

### Endianness
We use little-endian (`to_le_bytes`, `from_le_bytes`) to match x86 behavior and ensure cross-platform consistency.

### Buffer too small
FFI functions check buffer sizes and return -1 on failure.

## Completion Promise
When verification passes, output:
```
<promise>TASK_10D_COMPLETE</promise>
```

## Escape Hatch
If stuck after 10 iterations:
- Document blocking issues in `ralph-tasks/blocked/10d.md`
- Output: `<promise>TASK_10D_BLOCKED</promise>`
