# Task 03b: Surface Management

## Dependencies
- Task 03a must be complete (SDL2 window creation)
- Graphics module structure in place

## Context
The original game uses software surfaces for off-screen rendering and blitting. We need to implement an 8-bit indexed surface type that supports lock/unlock for direct pixel access and blit operations with clipping.

## Objective
Create a software surface implementation that supports the operations needed by the game's graphics code.

## Deliverables
- [ ] `platform/src/graphics/surface.rs` - Surface struct implementation
- [ ] Lock/Unlock for direct pixel access
- [ ] Blit with source rectangle and clipping
- [ ] Fill rectangle operation
- [ ] Clear operation
- [ ] FFI exports for surface operations

## Files to Create/Modify

### platform/src/graphics/surface.rs (NEW)
```rust
...
pub struct PlatformSurface(pub(crate) Surface);
...
pub extern "C" fn Platform_Surface_Create(width: i32, height: i32, bpp: i32) -> *mut PlatformSurface {
    ...
    Box::into_raw(Box::new(PlatformSurface(surface)))
}
...
pub extern "C" fn Platform_Surface_Blit(
    ...
    unsafe {
        match (*dest).0.blit(dx, dy, &(*src).0, sx, sy, sw, sh) {
            Ok(()) => 0,
            Err(e) => {
                set_error(e.to_string());
                -1
...
pub extern "C" fn Platform_Surface_BlitTransparent(
    ...
    unsafe {
        match (*dest).0.blit_transparent(dx, dy, &(*src).0, sx, sy, sw, sh) {
            Ok(()) => 0,
            Err(e) => {
                set_error(e.to_string());
                -1
...
grep -q "Platform_Surface_Create" ../include/platform.h && \
grep -q "Platform_Surface_Blit" ../include/platform.h && \
echo "VERIFY_SUCCESS"
```

## Implementation Steps
1. Create `platform/src/graphics/surface.rs`
2. Update `platform/src/graphics/mod.rs` to include surface module
3. Add surface FFI exports to `platform/src/ffi/mod.rs`
4. Run `cargo test` to verify surface tests pass
5. Run `cargo build --features cbindgen-run`
6. Verify header contains surface exports
...
void Platform_Surface_GetSize(const struct PlatformSurface *surface, int32_t *width, int32_t *height);
...
int32_t Platform_Surface_Blit(struct PlatformSurface *dest, int32_t dx, int32_t dy,
                              const struct PlatformSurface *src,
                              int32_t sx, int32_t sy, int32_t sw, int32_t sh);
int32_t Platform_Surface_BlitTransparent(struct PlatformSurface *dest, int32_t dx, int32_t dy,
                                         const struct PlatformSurface *src,
                                         int32_t sx, int32_t sy, int32_t sw, int32_t sh);
void Platform_Surface_Fill(struct PlatformSurface *surface, int32_t x, int32_t y,
...
### "cannot find type Surface in this scope"
- Ensure `pub mod surface;` is in graphics/mod.rs
- Ensure `pub use surface::Surface;` re-exports the type

### Blit produces garbage
- Check pitch calculation (pitch = width * bytes_per_pixel)
- Verify clipping logic handles negative coordinates

### Surface tests fail
- Ensure test creates surfaces with valid dimensions
- Check bounds checking in get_pixel/set_pixel

## Completion Promise
When verification passes (tests pass and header contains exports), output:
<promise>TASK_03B_COMPLETE</promise>

## Escape Hatch
If stuck after 12 iterations:
- Document blocking issue in `ralph-tasks/blocked/03b.md`
- Output: <promise>TASK_03B_BLOCKED</promise>
